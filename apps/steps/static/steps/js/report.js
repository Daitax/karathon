...
function openChangeReportForm() {
  ...
  let csrfToken = getCookie('csrftoken')
  let data = new FormData()
  data.append('window', 'open-change')
...
let openFormChangeReportButtons = document.querySelectorAll('[button-action="open-change-report-form"]')
if (openFormChangeReportButtons.length > 0) {
    openFormChangeReportButtons.forEach(function (button) {
      button.addEventListener('click', openChangeReportForm)
  })
}
