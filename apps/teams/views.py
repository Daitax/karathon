...
def open_add_desire_form(request):
    ...
    add_desire_window = render_to_string(
        "teams/popups/add_desire.html", context, request
    )
    ...
    if form.is_valid():
        ...
        try:
            ...
            if request.user.participant == desired_participant:
                ...
                return JsonResponse(out)
            if DesiredTeam.objects.filter(
                desirer=request.user.participant,
                desired_participant=desired_participant,
            ).exists():
                error_text = (
                    "* {} уже в твоей команде, выбери кого-то ещё".format(
                        desired_participant.first_name
                    )
                )
                context = {
                    "window": "form",
...
def index(request):
    ...
    desire_list = DesiredTeam.objects.select_related().filter(
        desirer__user=request.user.participant
    )
    context = {
...
def current_karathon_team(request):
    ...
    try:
        team = Team.objects.get(
            ...
            teamparticipant__participant=request.user.participant,
        )
